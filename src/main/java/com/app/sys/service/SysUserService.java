...
import com.app.sys.dao.SysFunctionDao;
import com.app.sys.dao.SysModuleDao;
import com.app.sys.dao.SysUserDao;
import com.app.sys.model.SysFunction;
import com.app.sys.model.SysModule;
import com.app.sys.model.SysRole;
...
public class SysUserService {
	...
	public int updateInfo(SysUser sysUser, String oldPass, String newPass) {
		int flag = 0;
		SysUser getUser = sysUserDao.get(sysUser.getUser_id());
		// 判断原密码是否为空，不为空则修改新密码
		if (StringUtil.isNotNullOrEmpty(oldPass)) {
			if (oldPass.equals(getUser.getPassword())) {
				sysUserDao.updatePass(sysUser.getUser_id(), newPass);
				flag = 1;
			} else {
				flag = 2;
			}
		...
		// sysUserDao.updaappatus(getUser.getId(), 1);
		// }
		return flag;
	}
	...
	public int updaappatus(int id, int status) {
		return sysUserDao.updaappatus(id, status);
	}
