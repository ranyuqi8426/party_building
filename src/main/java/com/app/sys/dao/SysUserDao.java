...
public class SysUserDao {
	...
	public int add(SysUser sysUser) {
		...
		String sql = "insert into t_sys_user(username,password,randomcode,status,realname,gender,mobile,phone, role_id,dwid,create_date,create_person,create_id)";
		sql += " values(:username,:password,:randomcode,1,:realname,:gender,:mobile,:phone,:role_id,:dwid,now(),:create_person,:create_id)";
	 ...
	 * @return
	 */
	public int updaappatus(int id, int status) {
	...
	public SysUser get(int id) {
		String sql = "select t.*,(select dwmc from es_sys_dw where dwid=t.dwid) dwmc"
				+ ",(select dwjb from es_sys_dw where dwid=t.dwid) dwjb" + " from t_sys_user t where user_id=?";
